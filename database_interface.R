...
con <- dbConnect(SQLite(), "lines_sequences.sqlite")

create_database(con, db_structure)

add_data(con, table="plate_data", file="importedData/plate_data.csv")
add_data(con, table="plate_tracking", file="importedData/plate_tracking.csv")
add_data(con, table="dictionary_phyla_id", file="importedData/dictionary_phyla_id.csv")
dbWriteTable(conn=con, value=read.csv("importedData/ufdb.csv"), name="ufdb",
...
WHERE plate_tracking.sent_on IS '2014-05-22';")
    dat <- fetch(res)
